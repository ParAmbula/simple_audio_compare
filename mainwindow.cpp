...
{
    m_settings=new QSettings(QApplication::applicationDirPath() + "/appsettings.ini",QSettings::IniFormat,this);
    if(m_settings->value("/Tran").toString()=="main_ru.qm" && m_settings->value("/qtLanSpec").toString()=="qt_ru")
    {
...
{
    m_settings->setValue("/qtLanSpec","qt_ru");
    m_settings->setValue("/Tran","main_ru.qm");
    QMessageBox::about(this,"Settings saved.","Restart application.");
}

void MainWindow::slotLangEN()
{
    m_settings->setValue("/qtLanSpec","qt_en");
    m_settings->setValue("/Tran","main_en.qm");
...
void MainWindow::slotRenameFile(QTreeWidgetItem *item)
{
    ...
    QString form="";
    int kol=oldname.length();
    tmp.chop(kol);
    QString newname=QInputDialog::getText(this,tr("Enter new name."),tr("New name:"),QLineEdit::Normal,"AudioFuzzy",&accept);
    tmp+=newname;
    if (accept && !newname.isEmpty())
    {
        if(file.fileName()!=newname)
        {
            foreach(auto l,AudioMask)
            {
                l.remove(0,1);
                if (item->text(2).endsWith(l))
                    form=l;
            }
            tmp+=form;
            newname+=form;
            if(file.rename(item->text(2),tmp))
            {
            ...
            {
                QMessageBox::warning(this,tr("Error"),tr("Type another name!"));
            }
        }
        else
